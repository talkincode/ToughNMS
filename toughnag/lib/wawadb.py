...
__github__ = 'https://github.com/onlytiancai/wawadb'
__author__ = 'onlytiancai'

default_data_dir = '/opt/nagios_perfdata/'
...
def test_read():
    db = WawaDB("localhost-service")
    begin_time = datetime.now() - timedelta(hours=24) 
    end_time = begin_time + timedelta(hours=24)
    results = list(db.get_data(begin_time, end_time, lambda x: x.find('User') != -1))
    print 'test_get_data get %s results %s --- %s ' % (len(results),begin_time,end_time )
